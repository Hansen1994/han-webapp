        ...
        <li class="item" v-for="(item, index) in lists" :key="'msg-' + index">
          <div class="content-item">
            <div class="flex">
              ...
              <div class="column">
                <div class="title">
                  {{ item.cuid ? item.cuid.name : '用户未设置昵称' }}
                </div>
...
export default {
  ...
  methods: {
    ...
    // 获取未读消息列表
    getMsgAll() {
      getMsg({ page: this.page, limit: this.limit }).then((res) => {
        if (res.code === 200) {
          this.lists = res.data
    ...
    // 获取点赞列表
    getHandsAll() {
      getHands({ page: this.page, limit: this.limit }).then((res) => {
        if (res.code === 200) {
          this.lists = res.data
        }
      })
